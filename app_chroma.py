...
"middle aged male singing in the middle of a park while being sad"
"middle aged female singing in the middle of a park while being sad" (Female)
"""
...
def gradio_search(query: str, model_type: str) -> str:
    ...
    for i, (doc, distance) in enumerate(zip(results['documents'][0], results['distances'][0])):
        ...
        try:
            ...
            # 'doc' is the combined "Title. Description" string
            ...
            if voice_info:
                ...
                # This line is FIXED to show the clean description
                output += f"   Description: {voice_info['description']}\n"
            else:
                # This block is running if database.py is not updated
                output += f"{i+1}. {doc}\n"
                output += f"   (LOOKUP FAILED: Please check database.py)\n" # Debug message
        except ImportError:
            output += f"{i+1}. {doc}\n"
    ...
    return output
# Initialize database with voice descriptions if available
def initialize_database():
    ...
    try:
        ...
        print("Could not import 'voice_titles' or 'voice_descriptions'. Database will be empty.")
    except Exception as e:
